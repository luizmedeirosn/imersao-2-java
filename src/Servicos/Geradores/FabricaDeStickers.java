...
public class FabricaDeStickers {
    private FabricaDeStickers() {}

    public static void criarSticker(InputStream inputStream, Filme filme) {
        ...
        Font fonte = new Font("Impact", Font.BOLD, (int) (altura * 0.08)); // tamanho da fonte de acordo com a altura da
                                                                           // imagem.
        ...
        System.out.println(filme.getTitulo() + ".png criado com sucesso!");
    }



    public static void criarSticker(InputStream inputStream, DadosDaGalaxia galaxia) {
        
        String nomeDoArquivo = galaxia.getTitulo()
                .replaceAll(":", "-")
                .replaceAll(" ", "-")
                .replaceAll("--", "-"); // windows não reconhece o ":" especiais para nomes de arquivos.
        ...
        try {
            icone = new ImageIcon( ImageIO.read( new FileInputStream("./.reacoes/selfie-paisagem.png")) );
        } catch (IOException e) {
            System.out.println("Não consegui carregar a imagem da selfie-paisagem.png");
        }
        icone
            .setImage(icone.getImage()
                .getScaledInstance( 
                    (int)(imagem.getWidth() * 0.6),
                    (int)(imagem.getHeight() * 0.6),
                    (int)(imagem.getHeight() * 0.6)
                )
            );
        ...
        graphics2D.drawImage(
                icone.getImage(),
                imagem.getWidth() - icone.getIconWidth(),
                imagem.getHeight() - icone.getIconHeight(),
                null);

        try {
            ImageIO.write(imagem, "png", new File("./.stickers/" + nomeDoArquivo + ".png"));
        } catch (Exception e) {
            System.out.println("O diretório para salvar a imagem não existe!\nCriei um novo...");
            try {
                new File("./.stickers/").mkdir(); // Gera a exceção FileNotFoundExeception, mas cria o diretorio.
        ...
        System.out.println(galaxia.getTitulo() + ".png criado com sucesso!");
    }
}
